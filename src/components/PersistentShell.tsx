...
export default function PersistentShell({ children }: PersistentShellProps) {
  ...
  const handleTabChange = useCallback((tab: TabId) => {
    // Try the registered handler (homepage scroll)
    const handled = dispatchTabChange(tab);
    if (handled) return;
    // Fallback: navigate to homepage with hash and preserve intended tab.
    sessionStorage.setItem(TARGET_HOME_TAB_KEY, tab);
    sessionStorage.removeItem(RESTORE_HOME_SCROLL_KEY);
    navigate(TAB_HREFS[tab], 'back');
  }, [dispatchTabChange, navigate]);

  // Always use button-driven navigation to keep transitions and state handoff consistent.
  const tabHrefMap = undefined;

  // Determine content padding mode based on route
  const isFullBleed = !isHomePage;
  const contentContainerClassName = isFullBleed
  ...
  return (
    <div className="bg-white content-stretch flex flex-col items-start relative size-full md:grid md:grid-cols-[repeat(4,_minmax(0px,_1fr))]">
      ...
      <div
        data-sidebar
        className="hidden content-stretch flex-col h-[200px] items-start justify-between relative shrink-0 w-full md:[grid-area:1_/_1] md:h-screen md:flex"
      >
        <div className="basis-0 box-border content-stretch flex flex-col grow items-start justify-between min-h-px min-w-px p-[8px] relative shrink-0 w-full">
          <LogoMain className="overflow-clip relative shrink-0 size-[32px]" onClick={handleLogoClick} />
